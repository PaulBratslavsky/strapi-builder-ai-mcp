/**
 * MCP Controller for Strapi Builder AI
 *
 ...
 */
function cleanupExpiredSessions(plugin: BuilderAiMcpPlugin, strapi: Core.Strapi): void {
  let cleaned = 0;
...
const mcpController = ({ strapi }: { strapi: Core.Strapi }) => ({
  ...
  async handle(ctx: any) {
    const plugin = strapi.plugin(PLUGIN_ID) as unknown as BuilderAiMcpPlugin;
